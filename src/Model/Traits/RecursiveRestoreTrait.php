...
namespace InworkNet\SDK\Model\Traits;

use InworkNet\SDK\Model\Types\AbstractCustomType;
use InworkNet\SDK\Model\Interfaces\RestorableInterface;
use InworkNet\SDK\Utils\AccessorUtil;
    ...
    private function getPropertyValueForRestore($type, $value, $field)
    {
        ...
        switch ($type) {
            ...
            case RestorableInterface::TYPE_FLOAT:
                if (!is_numeric($value)) {
                    throw new \UnexpectedValueException('Value is not float');
                }
            ...
            case RestorableInterface::TYPE_STRING:
                if (!is_string($value) && !is_numeric($value)) {
                    throw new \UnexpectedValueException('Value is not string');
                }
            ...
            case RestorableInterface::TYPE_ARRAY:
                if (!is_array($value)) {
                    throw new \UnexpectedValueException('Value is not array');
                }
                break;
